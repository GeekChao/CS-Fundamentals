...
class VendingMachine:
    ...
    def __init__(self, name, price):
        VendingMachine.items[name] = price
        self.name = name
        self.stock = 0
        self.amount = 0
    ...
    def deposit(self, amount):
        if self.stock <= 0:
            return 'Machine is out of stock. Here is your ${}.'.format(self.amount)
            
        self.amount += amount
        return 'Current balance: ${}'.format(self.amount)
    
    def vend(self):
        price = VendingMachine.items[self.name]
        
        if self.stock <= 0:
            return 'Machine is out of stock.'
        elif self.amount < price:
            return 'You must deposit ${} more.'.format(price - self.amount)
        
        result = 'Here is your {}.'.format(self.name) if self.amount == price else 'Here is your {} and ${} change.'.format(self.name, self.amount - price)
        self.amount -= price
        self.stock -= 1
...
class MissManners:
    ...
    def ask(self, message, *args):
        ...
        action = message[len(magic_word):]
        if hasattr(self.obj, action):
            getattr(self.obj, action)(*args)
        else:
            return 'Thanks for asking, but I know not {}.'.format(action)
