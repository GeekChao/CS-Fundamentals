...
public class Tree234 extends IntDictionary {
  ...
  public void insert(int key) {
    ...
    Tree234Node node = root;
    Tree234Node leafNode = node;
    boolean isVisited = false;
    
    while (node != null) {
      if(node.keys == 3 && !isVisited){ // 3-key node
        if(node.parent == null){  //root node
          Tree234Node rootNode = new Tree234Node(null, node.key2);
          root = rootNode;
          node = node.splitThreeKeyNode(root, 1);
          isVisited = true;
          continue;
        }else{
          int keyNum = node.addKeyInParent(node.parent, node.key2);
          node = node.splitThreeKeyNode(node.parent, keyNum);
          isVisited = true;
          continue;
        }
      }

      leafNode = node;  //stroe current node
