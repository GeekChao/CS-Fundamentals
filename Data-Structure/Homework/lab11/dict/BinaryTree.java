...
public class BinaryTree implements Dictionary {
  ...
  /*Warning!!!!  In this case, we cannot implement it recurvisely, since each stack
    frame would return a vaule and 'node variable' in find() function would get final one.*/
    /*if(node == null)
     return null;
    ...
    if(comp < 0){
      ...
      System.out.println(node.entry.value());
      return node;
    }
    return null;*/
    while(node != null){
  ...
  private void removeHelper(Comparable key, BinaryTreeNode node, int childNum){
    int comp = 0;
    if(node.parent != null)
       comp = key.compareTo(node.parent.entry.key());
    ...
    switch(childNum){
      ...
      case 1:
        ...
        break;
      case 2:
        // find smallest node in node's subtree
        BinaryTreeNode smallestNode = node.rightChild;
        while(smallestNode.leftChild != null)
          smallestNode = smallestNode.leftChild;
        if(smallestNode == node.rightChild){
          node.rightChild = null;
        }
        // deal with its right child
        if(smallestNode.rightChild != null){
          smallestNode.parent.leftChild = smallestNode.rightChild; 
          smallestNode.rightChild.parent = smallestNode.parent;
        }
        // replace its entry with node's entry
        node.entry.value = smallestNode.entry.value();
        node.entry.key = smallestNode.entry.key();
        // remove it 
        smallestNode.parent = null;
        smallestNode.rightChild = null;
        break;
      default:
        break;
    }
  ...
  public Entry remove(Object key) {
    ...
    removeHelper((Comparable)key, node, childNum);  
    size--;
    return node.entry;
  }
